document.addEventListener("DOMContentLoaded", () => {
  const content = {
    mps_one: {
      imgSrc: "images/College_Information/College_Logo1.png",
    ...
    mps_two: {
      imgSrc: "images/College_Information/College_Logo2.png",
    ...
    mps_three: {
      imgSrc: "images/College_Information/College_Logo3.png",
  ...
  spans.forEach((span, index) => {
    span.addEventListener("click", () => {
      clearInterval(interval); // Stop the auto-loop when a span is clicked
      activeIndex = index; // Update active index
      updateContent(activeIndex); // Update content immediately
      startAutoLoop(); // Restart auto-loop
    });
  });

  // Auto-loop through content every 3 seconds
  function startAutoLoop() {
    interval = setInterval(() => {
      activeIndex = (activeIndex + 1) % spans.length; // Increment index and loop
